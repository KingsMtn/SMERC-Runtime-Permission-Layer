...
- Result: Generated an AWS reviewer bundle with 5 action-chain examples, chain postcondition statuses {'gap': 1, 'pass': 4}, AWS postcondition statuses {'gap': 2, 'pass': 4}, and slowest local p95 4.254 ms.
...
| Report | Main Result |
| --- | --- |
| AWS agent action chain | scenarios=`5`, postures=`{'DENY': 3, 'THROTTLE': 2}` |
| AWS chain postcondition evidence | statuses=`{'gap': 1, 'pass': 4}` |
| AWS postcondition evidence | statuses=`{'gap': 2, 'pass': 4}` |
| Performance | status=`ready_for_local_review`, slowest_p95_ms=`4.254` |
