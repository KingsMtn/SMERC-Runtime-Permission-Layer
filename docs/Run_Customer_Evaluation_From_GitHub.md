...
3. Set `evaluation_set` to `both` to run the public examples, `cloud-admin` to run only the cloud administration pack, or `company-template` to run the company metadata template.
4. Start the run.
...
- the cloud-admin customer-evaluation action set
