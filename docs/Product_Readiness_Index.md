...
| Buyer-facing proof packet | Implemented as `docs/Buyer_Facing_Proof_Packet.md`, with cross-path hardening evidence and `docs/AWS_Shadow_Mode_Buyer_Example.md`. | Needs an external reviewer to say the packet is clear enough to evaluate. | One reviewer can understand problem, input, decision, report, pilot offer, and boundary from one artifact. | Use the proof packet in outreach and revise only from reviewer confusion. |
...
2. Use `docs/Buyer_Facing_Proof_Packet.md` as the short buyer proof artifact.
