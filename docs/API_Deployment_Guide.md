...
| `POST /v1/pipeline/evaluate` | Bearer | Evaluate hard gates, recoverability, execution routing, consequence-time authority, and settlement without executing the action |
...
When `SMERC_GITHUB_OIDC_TRUST` is configured, a matching GitHub Actions job can exchange one GitHub OIDC token at `POST /v1/auth/github` without a stored SMERC API key. The resulting session is scope-narrowed, workload-bound, and no longer lived than the source identity. See `GitHub_OIDC_Operations.md`.
...
- API keys remain a pilot compatibility credential model.
- GitHub OIDC removes a stored SMERC secret from matching Actions jobs, but it is not general enterprise federation and does not provide managed revocation.
- Scoped principals provide endpoint separation, and optional sessions provide bounded expiry; static bootstrap credentials still require managed storage where used.
