...
| --- | --- | --- | --- | --- |
| AWS AgentCore public security guidance | Agent runtimes need gateway-only paths, session/user binding, least privilege, credential scoping, command execution controls, and audit correlation. | AgentCore runtime, gateway, command, session, IAM, CloudTrail, CloudWatch, and VPC Flow Log metadata. | `docs/Public_Runtime_Pain_Points_To_SMERC.md`, `docs/AWS_Metadata_Intake_Contract.md`, `docs/AWS_Audit_Delay_And_Irreversibility_Map.md` | Add AgentCore-style gateway bypass, session binding, command authority, and credential exposure fields to AWS metadata examples. |
| MCP authorization and tool poisoning | MCP servers can create confused-deputy, token-passthrough, audience mismatch, tool metadata poisoning, and schema drift risk. | MCP authorization metadata, resource audience, token flow, tool schema, tool description, and tool-call envelope examples. | `docs/Public_Runtime_Pain_Points_To_SMERC.md`, `docs/MCP_Adversarial_Metadata_Replay.md`, `docs/Recoverability_Metadata_Contract.md` | Add delegated authority, token audience, and hidden-instruction normalization examples before tool execution. |
