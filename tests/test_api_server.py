...
class APIServerTests(unittest.TestCase):
    ...
    def test_schema_lists_versioned_endpoints_and_postures(self):
        ...
        self.assertIn("POST /v1/pipeline/evaluate", body["endpoints"])
        ...
        self.assertEqual(body["language_versions"]["decision_pipeline"], "smerc.decision-pipeline-contract.v1")
    ...
    def test_decision_pipeline_endpoint_requires_bearer_authentication(self):
        status, _, body = self.request_json(
            "/v1/pipeline/evaluate",
            method="POST",
            payload=DECISION_PIPELINE_EXAMPLE,
        )

        self.assertEqual(status, 401)
        self.assertEqual(body["error"], "authentication_required")

    def test_decision_pipeline_endpoint_settles_and_records_event(self):
        status, _, body = self.request_json(
            "/v1/pipeline/evaluate",
            method="POST",
            payload=DECISION_PIPELINE_EXAMPLE,
        ...
        self.assertEqual(status, 200)
        self.assertEqual(body["version"], "smerc.decision-pipeline-contract.v1")
        self.assertEqual(body["tenant_id"], "alpha")
        self.assertEqual(body["final_decision"], "SETTLE")
        self.assertTrue(body["should_execute"])
        self.assertTrue(body["should_commit"])
        self.assertEqual(body["authenticated_principal"]["tenant_id"], "alpha")
        self.assertEqual(
            body["runtime_observation"]["fail_behavior"],
            "deny_before_scoring_or_withhold_consequence_settlement",
        )

        events = self.request_json("/v1/security-events", key="alpha-secret")[2]["events"]
        self.assertIn("decision_pipeline.evaluated", {event["event_type"] for event in events})

    def test_decision_pipeline_endpoint_rejects_invalid_contract(self):
        payload = dict(DECISION_PIPELINE_EXAMPLE)
        ...
        self.assertEqual(body["error"], "invalid_decision_pipeline_request")
