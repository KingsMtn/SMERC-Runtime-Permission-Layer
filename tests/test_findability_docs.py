...
class FindabilityDocsTests(unittest.TestCase):
    ...
    def test_why_now_and_known_data_fallback_are_linked_and_bounded(self):
        why_now = (ROOT / "docs" / "Why_This_Matters_Now.md").read_text(encoding="utf-8")
        fallback = (ROOT / "docs" / "Known_Data_Fallback_Brief.md").read_text(encoding="utf-8")
        readme = (ROOT / "README.md").read_text(encoding="utf-8")
        ai_doc = (ROOT / "docs" / "AI_Readable_Reviewer_Bundle.md").read_text(encoding="utf-8")
        indexing = (ROOT / "docs" / "Public_Indexing_Assets.md").read_text(encoding="utf-8")
        public_fallback = (ROOT / "docs" / "Public_Evidence_Fallback_Plan.md").read_text(encoding="utf-8")
        front_door = (ROOT / "docs" / "Tier2_AWS_Reviewer_Front_Door.md").read_text(encoding="utf-8")
        changelog = (ROOT / "CHANGELOG.md").read_text(encoding="utf-8")
        bundle = json.loads((ROOT / "examples" / "ai_reviewer_bundle.json").read_text(encoding="utf-8"))

        self.assertIn("AI agents are moving from suggestion to execution", why_now)
        self.assertIn("If this action is already authorized", why_now)
        self.assertIn("recoverability-before-execution", why_now)
        self.assertIn("What This Does Not Prove", why_now)
        self.assertIn("Do not wait forever", fallback)
        self.assertIn("Never describe known-data fallback as customer validation", fallback)
        self.assertIn("Known-data fallback can strengthen technical proof", fallback)
        self.assertIn("docs/Why_This_Matters_Now.md", readme)
        self.assertIn("docs/Known_Data_Fallback_Brief.md", readme)
        self.assertIn("Why_This_Matters_Now.md", indexing)
        self.assertIn("Known_Data_Fallback_Brief.md", indexing)
        self.assertIn("Known Data Fallback Brief", ai_doc)
        self.assertIn("Known_Data_Fallback_Brief.md", public_fallback)
        self.assertIn("Why_This_Matters_Now.md", front_door)
        self.assertIn("why_this_matters_now", bundle["current_evidence"])
        self.assertIn("known_data_fallback_brief", bundle["current_evidence"])
        self.assertIn("known data fallback", bundle["search_and_index_terms"])
        self.assertIn("Why This Matters Now brief", changelog)
    ...
    def test_linux_foundation_standards_alignment_is_linked_and_bounded(self):
        text = (ROOT / "docs" / "Linux_Foundation_Standards_Alignment.md").read_text(encoding="utf-8")
        readme = (ROOT / "README.md").read_text(encoding="utf-8")
        ai_doc = (ROOT / "docs" / "AI_Readable_Reviewer_Bundle.md").read_text(encoding="utf-8")
        ai_bundle = json.loads((ROOT / "examples" / "ai_reviewer_bundle.json").read_text(encoding="utf-8"))

        self.assertIn("TRACE", text)
        self.assertIn("Agentic AI Foundation", text)
        self.assertIn("MCP", text)
        self.assertIn("AGENTS.md", text)
        self.assertIn("Agent Name Service", text)
        self.assertIn("pre-execution recoverability decision", text)
        self.assertIn("standards-adjacent, not standards-approved", text)
